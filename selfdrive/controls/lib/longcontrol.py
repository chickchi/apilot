...
def long_control_state_trans(
  ...
  # v1.5.7 lead-aware STOPPING release
  nearby_lead = (
    lead_status and
    0.0 < lead_d < 35.0
  )
...
class LongControl:
  ...
  def update(
    ...
    self.long_control_state, planned_stop = (
    ...
    # ---------------------------------------------------------------------
    # v1.4 smooth positive acceleration
    # ---------------------------------------------------------------------
    ...
    # - no gear command is ever transmitted
    # - braking / negative acceleration always bypasses the manager
    # =====================================================================
    self.upshift_limit_active = False
    self.upshift_cap = 0.0
    ...
    # v1.5.9:
    # 5th gear may still need one last human-like lift immediately before the
    # selected speed. Otherwise DV<=2.5 would shut the manager off and the car
    # could remain in 5th all the way to the target.
    ...
    if current_gear == 3:
      ...
      # v1.5.9:
      # Target actual 5->6 transition in roughly the 80+ km/h domain instead
      # of letting 5th wind out toward the selected speed.
      soft_min_v = 80.0
      hard_min_v = 76.0
    ...
    if current_gear == 3:
      demand_rpm_boost = (
        demand_rpm_boost_raw *
        0.80
      )

    elif current_gear in (
      4,
      5,
    ):
    ...
    # Keep aggressive downshift protection conservative.
    ...
    # v1.5.9:
    # G5 PRE_RELIEF/SHIFT may continue to DV=0.5 so a final 5->6 transition
    # can complete near the selected cruise speed.
    if (
    ...
    if self.upshift_state == 1:
      ...
      if gear_increased:
        ...
        # v1.5.9:
        # Low positive aEgo is no longer considered an automatic failure in G5.
        # Only actual deceleration after a meaningful interval cancels it.
    ...
    # ---------------------------------------------------------------------
      ...
      if gear_increased:
        ...
        if self.upshift_entry_gear == 5:
          # ===============================================================
          # v1.6.3 POSITIVE-TORQUE 5->6 PLATEAU + LAST-RESORT NUDGE
          #
          # Do not begin a normal 5->6 attempt with a near-coast request.
          # First hold a small but clearly positive acceleration plateau so
          # the car keeps gaining speed while engine load falls enough for
          # the stock TCU to consider 6th.  Only if the TCU remains G5>5 at
          # high RPM do we progressively deepen the SAME continuous relief.
          # There is no accelerator release/re-apply pulse and no gear CAN
          # command; the stock TCU remains the sole gear selector.
          # ===============================================================
          ...
          if rpm_valid:
            rpm_plateau_cap = interp(
              assist_rpm,
              [1950.0, 2150.0, 2300.0, 2450.0, 2600.0, 2750.0],
              [0.22, 0.22, 0.20, 0.18, 0.16, 0.14],
            )
          ...
          if self.g5_upshift_nudge_active:
            deep_lift_cap = interp(
              self.upshift_timer,
              [1.50, 2.20, 3.00, 3.80],
              [positive_plateau_cap, 0.13, 0.10, 0.08],
            )
          ...
          # Keep one continuous attempt long enough to avoid repeated
          # accelerator on/off cycling. Meaningful deceleration still aborts.
          shift_timeout = 4.20
        ...
        if self.upshift_entry_gear == 5:
          # During the intentional lift, aEgo approaching zero is expected.
          # Only meaningful actual deceleration aborts the 5->6 attempt.
          weak_response = (
            self.upshift_timer >
            weak_check_time and
            CS.aEgo <
        ...
        if (
          ...
          # v1.6.3: after one long monotonic attempt, retry soon enough that
          # 5th cannot remain stranded to the cruise target.  The short rest
          # also avoids a visible accelerator re-apply interval.
          if failed_gear == 5:
            self.upshift_cooldown = (
              1.00
              if self.g5_upshift_nudge_active
