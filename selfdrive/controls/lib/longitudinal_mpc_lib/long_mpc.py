...
MAX_ACCEL = 2.5
T_FOLLOW = 1.45
COMFORT_BRAKE = 2.0 #defualt 2.5
STOP_DISTANCE = 6.5

# GAP4 = AUTO.
# Lead/radar/vision state is intentionally NOT used to choose the target gap.
# This prevents a lead acquisition/source switch from changing dRel/vRel and
# t_follow in the same frame.
AUTO_GAP_SPEED_BP_KPH = [0.0, 30.0, 70.0, 110.0]
AUTO_GAP_TF_V = [1.10, 1.20, 1.35, 1.50]
AUTO_GAP_TF_UP_RATE = 0.12
AUTO_GAP_TF_DOWN_RATE = 0.06
...
class LongitudinalMpc:
  def __init__(self, mode='acc'):
    ...
    self.autoGapActive = False
    self.autoGapTarget = T_FOLLOW
  ...
  def get_cost_multipliers(self, v_lead0, v_lead1, primary_pullaway=False):
    ...
    elif (v_lead0 - v_ego >= 0) and (v_lead1 - v_ego >= 0):
  ...
  def update(self, carstate, radarstate, model, controls, v_cruise, x, v, a, j, y,
             prev_accel_constraint, reset_state, clear_road_recovery=False):
    ...
    v_cruise, stop_x, self.mode = self.update_apilot(
      controls, carstate, radarstate, model, v_cruise, self.mode)
    ...
    if self.mode == 'acc':
      ...
      x_obstacles = np.column_stack(
        [lead_0_obstacle, lead_1_obstacle, cruise_obstacle, x2])
      ...
      self.debugLongText1 = (
        ...
        .format(
          ...
          lead_0_obstacle[0],
          self.stopDist,
          self.restart_stop_distance,
          self.restart_gap_capture,
          int(self.restart_gap_active),
          int(self.clear_road_recovery),
        )
        + " AG{} TF{:.2f}/{:.2f}".format(
          int(self.autoGapActive),
          self.t_follow,
          self.autoGapTarget,
        )
      )
      ...
      self.debugLongText1 = (
        "A{:3.2f},L0{:5.1f},C{:5.1f},X{:5.1f},F{:5.1f}"
        .format(self.max_a, lead_0_obstacle[0], cruise_target[0], x[0], xforward[0])
      )
    ...
    if (
      np.any(lead_xv_0[FCW_IDXS,0] - self.x_sol[FCW_IDXS,0] < CRASH_DISTANCE) and
  ...
  def update_params(self):
    ...
    if self.lo_timer > 200:
      ...
      self.trafficStopDistanceAdjust = (
        float(int(Params().get("TrafficStopDistanceAdjust", encoding="utf8"))) / 100.)
    elif self.lo_timer == 60:
      self.applyLongDynamicCost = Params().get_bool("ApplyLongDynamicCost")
      self.trafficStopAccel = (
        float(int(Params().get("TrafficStopAccel", encoding="utf8"))) / 100.)
      ...
      self.stopDistance = (
        float(int(Params().get("StopDistance", encoding="utf8"))) / 100.)
    elif self.lo_timer == 100:
      self.tFollowSpeedRatio = (
        float(int(Params().get("TFollowSpeedRatio", encoding="utf8"))) / 100.)
      self.tFollowGap1 = (
        float(int(Params().get("TFollowGap1", encoding="utf8"))) / 100.)
      self.tFollowGap2 = (
        float(int(Params().get("TFollowGap2", encoding="utf8"))) / 100.)
      self.tFollowGap3 = (
        float(int(Params().get("TFollowGap3", encoding="utf8"))) / 100.)
      self.tFollowGap4 = (
        float(int(Params().get("TFollowGap4", encoding="utf8"))) / 100.)
      ...
      self.trafficStopAdjustRatio = (
        float(int(Params().get("TrafficStopAdjustRatio", encoding="utf8"))) / 100.)

  def update_gap_tf(self, controls, v_ego, a_ego):
    v_ego_kph = v_ego * CV.MS_TO_KPH
    self.applyCruiseGap = int(clip(controls.longCruiseGap, 1, 4))

    if self.openpilotLongitudinalControl:
      if self.applyCruiseGap == 4:
        self.autoGapActive = True

        auto_tf_base = interp(
          v_ego_kph,
          AUTO_GAP_SPEED_BP_KPH,
          AUTO_GAP_TF_V,
        )

        self.autoGapTarget = max(
          0.6,
          auto_tf_base * (2.0 - self.mySafeModeFactor),
        )

        if self.autoGapTarget > self.t_follow:
          self.t_follow = min(
            self.autoGapTarget,
            self.t_follow + AUTO_GAP_TF_UP_RATE * DT_MDL,
          )
        else:
          self.t_follow = max(
            self.autoGapTarget,
            self.t_follow - AUTO_GAP_TF_DOWN_RATE * DT_MDL,
          )

      else:
        self.autoGapActive = False
        self.autoGapTarget = self.t_follow
        ...
        if v_ego_kph >= self.v_ego_kph_prev:
          ...
          cruiseGapRatio = interp(
            v_ego_kph,
            [0, 100],
            [tf, tf * self.tFollowSpeedRatio],
          )

          self.t_follow = max(
            0.6,
            cruiseGapRatio * (2.0 - self.mySafeModeFactor),
          )
      ...
      if self.status:
        if v_ego_kph < 0.1:
          ...
          self.applyCruiseGap = int(
            interp(
              a_ego,
              [-1.5, -0.5],
              [4, self.applyCruiseGap],
            )
          )
  ...
  def update_apilot(self, controls, carstate, radarstate, model, v_cruise, mode):
    ...
    if self.xState == XState.softHold:
      ...
      if carstate.gasPressed:
        ...
        if v_ego < 0.1:
          if (
            self.trafficState == 2 and
            (
              not self.trafficError or
              (self.trafficError and cruiseButtonCounterDiff > 0)
            )
          ):
            self.xState = XState.e2eCruisePrepare
            ...
            if self.trafficState == 2:
              ...
              stop_dist = (
                ...
                interp(
                  self.xStop,
                  [0, 100],
                  [1.0, self.trafficStopAdjustRatio],
                )
              )
      ...
      if controls.longActiveUser <= 0:
        ...
        (carstate.brakePressed or cruiseButtonCounterDiff < 0) and
        self.e2eCruiseCount > 0
      ):
        self.xState = XState.e2eStop
        self.stopDist = 2.0
        ...
        v_ego_kph < 2.0 and
        (self.trafficState != 2 or cruiseButtonCounterDiff > 0)
      ):
        self.xState = XState.e2eStop
        self.stopDist = 2.0
    ...
    if self.trafficStopMode > 0:
      if self.trafficStopMode == 3:
        vision_detected = (
          radarstate.leadOne.dRel < 90 and
          radarstate.leadOne.status and
          not radarstate.leadOne.radar
        )
        mode = (
          'blended'
          if (
            self.xState in [XState.e2eCruisePrepare] or
            vision_detected
          )
          else 'acc'
        )
        ...
        mode = (
          'blended'
          if self.xState in [XState.e2eCruisePrepare]
          else 'acc'
        )

      else:
        if (
          self.xState == XState.e2eCruisePrepare or
          (
            self.xState == XState.e2eStop and
            self.stopDist > 40
          )
        ):
          mode = 'blended'
