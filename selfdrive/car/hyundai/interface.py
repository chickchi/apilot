...
# -------------------------------------------------------------
# v1.8 Legacy HKG MAIN button
# -------------------------------------------------------------
MAIN_BUTTONS_DICT = {
  1: ButtonType.altButton3,
}
...
class CarInterface(CarInterfaceBase):
  ...
  def _get_params(
    ...
    ret.radarUnavailable = (
      RADAR_START_ADDR not in fingerprint[1] or
      DBC[ret.carFingerprint]["radar"] is None
    )
    ...
    if candidate in CANFD_CAR:
      if Ecu.adas in [
        ...
        ret.flags |= (
          HyundaiFlags.CANFD_HDA2.value
        )

      else:
        if 0x1cf not in fingerprint[4]:
          ret.flags |= (
            HyundaiFlags.CANFD_ALT_BUTTONS.value
          )
      ...
      if 0x485 in fingerprint[2]:
        ret.flags |= (
          HyundaiFlags.SEND_LFA.value
        )

      if (
        0x38d in fingerprint[0] or
        0x38d in fingerprint[2]
      ):
        ret.flags |= (
          HyundaiFlags.USE_FCA.value
        )
    ...
    if candidate in CANFD_CAR:
      ret.longitudinalTuning.kpV = [
        0.1
      ]

      ret.longitudinalTuning.kiV = [
        0.0
      ]
      ...
      ret.enableBsm = (
        0x58b in fingerprint[0]
      )

      ret.sccBus = (
        2
        if 1056 in fingerprint[2]
        else 0
      ...
      ret.hasLfaHda = (
        1157 in fingerprint[0] or
        1157 in fingerprint[2]
      )

      if (
        ret.sccBus == 2 and
        candidate not in CAMERA_SCC_CAR
      ):
    ...
    if candidate in CANFD_CAR:
      ret.safetyConfigs = [
        ...
        get_safety_config(
          car.CarParams.SafetyModel.hyundaiCanfd
        ),
      ]
      ...
      if candidate in LEGACY_SAFETY_MODE_CAR:
        ...
        ret.safetyConfigs = [
          get_safety_config(
            car.CarParams.SafetyModel.hyundai,
            0,
          )
        ]
    ...
    if candidate in (
      ...
      ret.flags |= (
        HyundaiFlags.ALT_LIMITS.value
      )

      ret.safetyConfigs[
        -1
      ].safetyParam |= (
        Panda.FLAG_HYUNDAI_ALT_LIMITS
      )
    ...
    if Params().get_bool(
      ...
      ret.safetyConfigs[
        ...
        Panda.FLAG_HYUNDAI_AUTO_ENGAGE
      )
  ...
  def _update(self, c):
    ...
    if (
      ...
      # Handle button changing while another cruise button is pressed.
      if (
        self.CS.cruise_buttons[-1] != 0 and
    ...
    if self.low_speed_alert:
      events.add(
        car.CarEvent.EventName.belowSteerSpeed
      )
